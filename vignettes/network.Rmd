...
```{r}
names(potter)[names(potter)=="fam"]<- "oldfam"

ds <- ped2fam(potter, famID = "famID", personID = "ID") #this line provides the name for the new family ID variable created by the ped2fam function
...
`r add[1,11]` of their nuclear DNA.
...
#what is happening here? When I run this code, add_list is a list of 0...
```
...
Here we calculate the mitochondrial relatedness between a subset of
...
as person 1 and person 2, as designated by `r mit[1,2]` in the table, whereas person 1 and person 3 do
not (`r mit[1,3]`).
